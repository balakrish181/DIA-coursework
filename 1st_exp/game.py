...
class Battle:
    ...
    def rl_space(self, action):
        reward = 0
        #self.reward = PENALTY_TIME_STEP  # Penalty for each time step
        done = False
        ...
        if action == 0:
            ...
            self.fire_bullet()
            # if self.bullet_state == 1:
            #     self.reward += PENALTY_WASTED_BULLET  # Penalty for wasting a bullet
        ...
        #     self.reward += REWARD_CLOSE_TO_TARGET
